 ...
 *
 * This header provides declarations for managing the keyboard of the PicoCalc
...
/**
 * @brief Tests if a specific key is pressed, with a wait until the key is
 * pressed.
 *
 * @param key The key to test (use values from MY_KEY).
 * @return Non-zero if the key is pressed, 0 otherwise.
...
/**
 * @brief Gets the character corresponding to the pressed key.
 *
 * @return The character corresponding to the pressed key, or 0 if no key is
 * pressed.
 */
