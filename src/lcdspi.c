...
static short current_x = 0, current_y = 0;  // the current default position for
                                            // the next char to be written
...
void __not_in_flash_func(spi_write_fast)(spi_inst_t *spi, const uint8_t *src,
                                         size_t len) {
...
void draw_buffer_spi(int x1, int y1, int x2, int y2, unsigned char *p) {
    ...
    while (i--) {
        c.rgbbytes[0] =
            *p++;  // this order swaps the bytes to match the .BMP file
        c.rgbbytes[1] = *p++;
        c.rgbbytes[2] = *p++;
...
void draw_bitmap_spi(int x1, int y1, int width, int height, int scale,
    ...
    // Adjust coordinates if part of the bitmap is outside the displayable area
    vertCoord = y1;
    if (y1 < 0) y1 = 0;  // Adjust for y-coord above the top of the screen
    XStart = x1;
    if (XStart < 0) XStart = 0;  // Adjust for x-coord to the left of the screen
    XEnd = x1 + (width * scale) - 1;
    if (XEnd >= hres)
        XEnd = hres - 1;  // Adjust for width extending beyond the right margin
    YEnd = y1 + (height * scale) - 1;
    if (YEnd >= vres)
        YEnd =
            vres - 1;  // Adjust for height extending beyond the bottom margin

    // Define the region for the bitmap
    define_region_spi(XStart, y1, XEnd, YEnd, 1);

    n = 0;
    for (i = 0; i < height; i++) {  // Iterate through each line of the bitmap
        for (j = 0; j < scale; j++) {  // Scale the lines vertically
            if (vertCoord++ < 0)
                continue;  // Skip lines above the top of the screen
            if (vertCoord >
                vres) {  // Stop if we exceed the bottom of the screen
                lcd_spi_raise_cs();  // Set CS high
                return;
            }
            horizCoord = x1;
            for (k = 0; k < width;
                 k++) {  // Iterate through each pixel in the line
                for (m = 0; m < scale; m++) {  // Scale the pixels horizontally
                    if (horizCoord++ < 0)
                        continue;  // Skip pixels to the left of the screen
                    if (horizCoord > hres)
                        continue;  // Skip pixels beyond the right margin
                    if ((bitmap[((i * width) + k) / 8] >>
                         (((height * width) - ((i * width) + k) - 1) % 8)) &
                        1) {
                        hw_send_spi((uint8_t *)&f, 2);  // Send foreground color
                    } else {
                        hw_send_spi((uint8_t *)&b, 2);  // Send background color
                    }
                    n += 3;
                }
            }
...
void draw_rect_spi(int x1, int y1, int x2, int y2, uint16_t col) {
    // convert the colours to 565 format
    // unsigned char col[3];
    // uint16_t col = RGB565((c >> 16), (c >> 8) & 0xFF, (c & 0xFF));
    ...
    if (x1 == x2 && y1 == y2) {
        ...
        for (y = y1; y <= y2; y++) {
            spi_write_fast(Pico_LCD_SPI_MOD, (const uint8_t *)lcd_buffer,
                           i * 2);
        }
#endif
    }
 ...
 * The char is printed at the current location defined by current_x and
 *current_y
 *****************************************************************************************/
...
void scroll_lcd_spi(int lines) {
    ...
    if (lines >= 0) {
        ...
        draw_rect_spi(0, vres - lines, hres - 1, vres - 1,
                      gui_bcolour);  // erase the lines to be scrolled off
    } else {
        lines = -lines;
        ...
        draw_rect_spi(0, 0, hres - 1, lines - 1,
                      gui_bcolour);  // erase the lines introduced at the top
    }
}

void display_put_c(char c) {
    // if it is printable and it is going to take us off the right hand end of
    // the screen do a CRLF
    ...
    switch (c) {
        case '\b':
            ...
            if (current_x < 0) {               // Go to end of previous line
                ...
                current_x =
                    (s_width - 1) * gui_font_width;  // go to last character
            }
            return;
        case '\r':
            current_x = 0;
...
///////=----------------------------------------===//////
void lcd_clear() { draw_rect_spi(0, 0, hres - 1, vres - 1, 0x0000); }
...
void pico_lcd_init() {
    ...
    spi_write_data(0x55);  // 18 bit colour for SPI
